 ...
 * at least 1. The first and third passwords are valid: they contain one a or
...
// Time: O(2n) => O(n);
// Space: O(1);
const passwords = require('./day2Input.json');

const countValidPasswords = (arr) => {
  let validPasswords = 0;

  for (let i = 0; i < arr.length; i++) {
    let [count, pass] = arr[i];
    let [nums, char] = count.split(' ');
    let [min, max] = nums.split('-');

    let charCount = countLetters(char, pass);

    if (charCount >= Number(min) && charCount <= Number(max)) validPasswords++;
  }
 ...
 * password policy rules from his old job at the sled rental place
...
const countValidPasswordsTwo = (arr) => {
  ...
  for (let i = 0; i < arr.length; i++) {
    let [indices, pass] = arr[i];
    let [nums, char] = indices.split(' ');
    let [lo, hi] = nums.split('-');

    if ((pass[lo - 1] === char) ^ (pass[hi - 1] === char)) validPasswords++;
  }
...
countValidPasswordsTwo(passwords);
