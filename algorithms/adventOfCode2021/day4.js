 ...
 * row or column of marked numbers (in this case, the entire top row is marked: 14 21 17 24 4).
 * 
...
const checkCardRowsAndColumns = (card, row, col) => {
  let rowIdx = row + 1;
  let colIdx = col + 1;
  let rowCount = 0;
  let colCount = 0;
  ...
  // check vertical
  while (rowIdx % 5 !== row) {
    if (typeof card[rowIdx % 5][col] === 'string') {
      rowCount++;
    }
    rowIdx++;
  }
  ...
  // check horizontal
  while (colIdx % 5 !== col) {
    if (typeof card[row][colIdx % 5] === 'string') {
      colCount++;
    }
    colIdx++;
  }
...
const checkCardsForMatch = (num, cardsArray) => {
  for (let i = 0; i < cardsArray.length; i++) {
    let bingoCard = cardsArray[i];

    for (let row = 0; row < bingoCard.length; row++) {
      for (let col = 0; col < bingoCard[0].length; col++) {
        if (bingoCard[row][col] === num) {
          bingoCard[row][col] = bingoCard[row][col].toString();

          let isBingo = checkCardRowsAndColumns(bingoCard, row, col);
