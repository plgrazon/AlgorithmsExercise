...
const countTrees = (x = 3, y = 1) => {
  let rowIndex = 0;
  let colIndex = 0;
  let totalTrees = 0;

  while (rowIndex < matrix.length) {
    if (matrix[rowIndex][colIndex % matrix[0].length] === '#') {
      totalTrees++;
    }
...
const countTreesMultipleSlopes = () => {
  let totalCount = slopes.reduce((product, { x, y }) => {
    return product * countTrees(x, y);
  }, 1);
