...
const distributeCandies = function(candies, num_people) {
  ...
  while (candies) {
    result[idx] = Math.min(candies, count) + (result[idx] || 0);

    if (candies - count <= 0) break;
...
// Time: O(n);
// Space: O(n);
const distributeCandiesLong = function(candies, num_people) {
  let count = 1;
  let idx = 0;
  let result = new Array(num_people).fill(0);
  ...
  while (candies) {
    if (candies - count <= 0) {
      result[idx] = candies + (result[idx] || 0);
      break;
    } else {
      result[idx] = count + (result[idx] || 0);
      candies -= count;
      count++;
    }
